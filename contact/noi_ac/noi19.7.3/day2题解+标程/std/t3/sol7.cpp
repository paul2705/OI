...
int main()
{
	freopen("data\\x7.in","r",stdin);
	freopen("data\\x7.out","w",stdout);
	int m=in;
	fo0(i,11)in,buf;
	int pc=n*3,S=5e4,T=S+1,rf=0;
	fo1(i,n/2)
	{
		flow::add(S,i,1);
		flow::add(S,i+n,1);
		flow::add(S,i+n*2,1);
		flow::add(i+n/2,T,1);
		flow::add(i+n/2+n,T,1);
		flow::add(i+n/2+n*2,T,1);
		rf+=3;
	}
	std::vector<pii>res;
	fo1(i,m)
	{
		...
		fo0(j,c)
		{
			...
			flow::add(t.xx,a,1);
			flow::add(t.xx+n,a,1);
			flow::add(t.xx+n*2,a,1);
			res.pb(mp(flow::add(b,a,1),i));
			flow::add(b,t.yy,1);
			flow::add(b,t.yy+n,1);
			flow::add(b,t.yy+n*2,1);
			flow::add(S,b,1);
			flow::add(a,T,1);
		}
	}
	...
	//if(fl==rf)while(1);
	std::map<int,int>ans;
	foe(i,res)if(flow::e[i->xx].w)ans[i->yy]++;
	fo1(i,m)out,ans[i],'\n';
}
